...
import { PL_CURRICULUM as CURRICULUM, PL_ALL_UNITS as ALL_UNITS, plUnitById as unitById } from '../../lib/polishCurriculum';

// ─────────────────────────────────────────────────────────────────────────────
//  Polski — corso di polacco gamificato e adattivo (stesso motore di French.jsx).
...
const LS = {
  progress: 'shadow_monarch_pl_progress',   // { [unitId]: {done,bestScore,seenAt} }
  cache: 'shadow_monarch_pl_cache_v1',      // lezioni personalizzate sul profilo adattivo
  learner: 'shadow_monarch_pl_learner_v1',  // profilo adattivo: errori, abilità, velocità
  srs: 'shadow_monarch_pl_srs',             // { [itemKey]: {box,dueTs,lapses,seen} }
  streak: 'shadow_monarch_pl_streak',       // { count, lastDate }
  daily: 'shadow_monarch_pl_daily',         // { date, xp }
  log: 'shadow_monarch_pl_log',             // [ {unitId,item,correct,ms,ts} ] (cap 500)
};
...
const Polish = ({ playerStats, setPlayerStats, soundEnabled = true, soundTheme = 'solo' }) => {
  ...
  const cleanLesson = (parsed, unit) => {
    const vocab = Array.isArray(parsed?.vocab)
      ? parsed.vocab
        .map((v) => ({ w: String(v?.w || '').trim(), it: String(v?.it || '').trim(), note: String(v?.note || '').trim() }))
        .filter((v) => v.w && v.it)
        .filter((v, i, all) => all.findIndex((x) => x.w.toLowerCase() === v.w.toLowerCase()) === i)
        .slice(0, 8)
      : [];
  ...
  const buildOfflineLesson = (unit) => {
    const vocab = [
      ['cześć', 'ciao'], ['proszę', 'per favore / prego'], ['dziękuję', 'grazie'],
      ['chciałbym', 'vorrei (uomo)'], ['gdzie jest… ?', 'dov\'è… ?'], ['ile to kosztuje?', 'quanto costa?'],
      ['nie rozumiem', 'non capisco'], ['czy może pan/pani powtórzyć?', 'può ripetere?'],
    ].map(([w, it]) => ({ w, it, note: '' }));
    const exercises = [
      { type: 'mc', q: 'Come dici “vorrei” in polacco (uomo che parla)?', options: ['Chciałbym', 'Jestem', 'Mam', 'Idę'], answer: 'Chciałbym', explain: 'Chciałbym è la forma cortese maschile per chiedere qualcosa (chciałabym al femminile).' },
      { type: 'mc', q: 'Scegli la frase per chiedere dove si trova qualcosa.', options: ['Gdzie jest… ?', 'Ile to kosztuje?', 'Do widzenia!', 'Przepraszam.'], answer: 'Gdzie jest… ?', explain: 'Gdzie jest…? significa “Dov\'è…?”.' },
      { type: 'mc', q: 'Come chiedi il prezzo?', options: ['Ile to kosztuje?', 'Jak się nazywasz?', 'Jak się masz?', 'Jest południe.'], answer: 'Ile to kosztuje?', explain: 'Ile to kosztuje? è la formula naturale per chiedere il prezzo.' },
      { type: 'type', q: 'Traduci in polacco: “Buongiorno, per favore.”', answer: 'Dzień dobry, proszę.', explain: 'Dzień dobry è il saluto formale di giorno; proszę accompagna una richiesta cortese.' },
      { type: 'type', q: 'Traduci in polacco: “Non capisco.”', answer: 'Nie rozumiem.', explain: 'La negazione "nie" va sempre prima del verbo.' },
      { type: 'type', q: 'Traduci in polacco: “Può ripetere?”', answer: 'Czy może pan powtórzyć?', explain: 'Czy introduce la domanda; pan/pani è la forma di cortesia.' },
      { type: 'mc', q: 'Al ristorante, cosa significa “rachunek”?', options: ['Il conto', 'La prenotazione', 'Il menu', 'La camera'], answer: 'Il conto', explain: 'Rachunek è il conto al ristorante; poproszę rachunek = il conto, per favore.' },
      { type: 'type', q: 'Traduci in polacco: “Grazie, arrivederci.”', answer: 'Dziękuję, do widzenia.', explain: 'Do widzenia è il saluto standard formale per congedarsi.' },
      { type: 'mc', q: 'Quale frase è più adatta per chiedere aiuto?', options: ['Czy może mi pan pomóc?', 'Chciałbym spać.', 'Jest poniedziałek.', 'Mam dwadzieścia lat.'], answer: 'Czy może mi pan pomóc?', explain: 'Czy może mi pan/pani pomóc? significa “Può aiutarmi?”.' },
    ];
    return { version: LESSON_VERSION, title: `${unit.title} — base pratica`, objective: `Usare frasi essenziali per ${unit.topic}.`, explanation: 'Lezione di emergenza offline: frasi brevi, cortesi e riutilizzabili nella vita reale.', pronunciation: '', examples: ['Dzień dobry, poproszę kawę. — Buongiorno, vorrei un caffè, per favore.'], vocab, exercises, generatedAt: new Date().toISOString(), offline: true };
  };
  ...
  const generateLesson = async (unit, compact = false) => {
    ...
    const sys = `Sei un docente di polacco madrelingua, specializzato in studenti italiani.
Progetta UNA micro-lezione eccellente sul tema "${unit.topic}" per livello ${unit.level}.
Deve insegnare una cosa precisa, farla usare subito e correggere gli errori tipici degli italiani (es. i 7 casi, i 3 generi, l'aspetto verbale perfettivo/imperfettivo).
...
{"title":"titolo breve","objective":"obiettivo misurabile in italiano","explanation":"regola chiara in italiano con 1-2 errori tipici da evitare","pronunciation":"nota di pronuncia solo se utile (es. ł, sz, cz, rz)","examples":["frase polacca — traduzione italiana"],"vocab":[{"w":"espressione polacca","it":"traduzione italiana","note":"genere/caso/aspetto o pronuncia, breve"}],"exercises":[
 {"type":"mc","skill":"vocabolo/grammatica/conversazione/viaggio","q":"consegna italiana non ambigua","options":["4 risposte polacche"],"answer":"una delle options, identica carattere per carattere","explain":"perché è corretta e perché l'errore tipico è sbagliato"},
 {"type":"type","skill":"vocabolo/grammatica/conversazione/viaggio","q":"Traduci in polacco: frase italiana naturale","answer":"risposta polacca esatta","explain":"correzione breve"}
]}
Regole obbligatorie: esattamente 8 vocaboli e 9 esercizi; almeno 3 type e almeno 3 mc; ogni mc ha una sola risposta corretta e answer coincide ESATTAMENTE con una option; non usare domande-trabocchetto o traduzioni multiple valide; usa i diacritici polacchi corretti (ą,ć,ę,ł,ń,ó,ś,ź,ż); frasi brevi e realistiche; non ripetere lo stesso vocabolo o esercizio; spiegazioni in italiano, risposte/esempi in polacco.\nTema: ${unit.title} — ${unit.topic}.`;
    const data = await requestSystemAI({
      // Llama 3.3 70B è rapido e stabile anche sul polacco; il proxy prova poi
      // Agnes/GPT-OSS/NVIDIA se Groq è in rate-limit o senza chiave.
      model: getModelFor('polish') || 'groq:llama-3.3-70b-versatile',
      temperature: 0.35,
  ...
  const accentBar = (
    <div className="flex h-1.5 w-full overflow-hidden rounded-full">
      <div style={{ background: '#f2f2f2', flex: 1 }} /><div style={{ background: '#dc143c', flex: 1 }} />
    </div>
  );
  ...
  if (view === 'lesson' && queue[idx]) {
    ...
    return (
      <div className="p-4 pb-24 min-h-full" style={{ background: 'radial-gradient(120% 80% at 50% -10%, rgba(220,20,60,0.14), transparent 60%)' }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg,#dc143c,#8f0f2c)' }} animate={{ width: `${pct}%` }} />
          </div>
          <span className="text-[11px] font-mono text-gray-400">{idx + 1}/{queue.length}</span>
        </div>

        <p className="text-[10px] uppercase tracking-widest text-red-300/80 mb-1">{activeUnit?.emoji} {activeUnit?.title}</p>
        <h2 className="text-lg font-black text-white mb-4 leading-snug" style={{ fontFamily: 'Russo One, sans-serif' }}>{ex.q}</h2>

        {idx === 0 && lessonGuide && (
          <div className="mb-4 rounded-2xl p-3 space-y-2" style={{ background: 'rgba(220,20,60,0.12)', border: '1px solid rgba(220,20,60,0.35)' }}>
            {lessonGuide.offline && <p className="text-[10px] uppercase tracking-widest text-amber-300">Modalità offline · contenuto base</p>}
            <p className="text-[10px] uppercase tracking-widest text-red-200">Obiettivo</p>
            ...
            {lessonGuide.examples?.length > 0 && <p className="text-[11px] text-red-100 italic">{lessonGuide.examples[0]}</p>}
            {lessonGuide.vocab?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {lessonGuide.vocab.slice(0, 4).map((v) => <span key={v.w} className="text-[10px] px-2 py-1 rounded-lg text-gray-200" style={{ background: 'rgba(255,255,255,0.08)' }}>{v.w} · {v.it}</span>)}
              </div>
            )}
          </div>
        )}
        ...
        {ex.type === 'mc' ? (
              ...
              return (
                <button key={opt} disabled={!!feedback} onClick={() => setPicked(opt)}
                  ...
                  style={reveal
                    ...
                    : isPicked ? { background: 'rgba(220,20,60,0.22)', borderColor: '#dc143c', color: '#fff' }
          ...
          <input autoFocus value={answer} disabled={!!feedback}
            ...
            placeholder="Scrivi in polacco…"
            className="w-full px-4 py-3 rounded-xl bg-black/40 border text-white text-sm focus:outline-none"
            style={{ borderColor: feedback === 'ok' ? '#22b07d' : feedback === 'ko' ? '#e0243a' : 'rgba(220,20,60,0.5)' }} />
        )}
          ...
          {feedback && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              ...
              <p className="text-sm font-black" style={{ color: feedback === 'ok' ? '#7ff0c0' : '#ff9fab' }}>
                {feedback === 'ok' ? '✓ Dokładnie!' : `✗ Risposta: ${ex.answer}`}
              </p>
        ...
        <button onClick={feedback ? next : submit}
          ...
          style={feedback
            ? { background: 'linear-gradient(135deg,#dc143c,#8f0f2c)', color: '#fff' }
            : (picked || answer.trim()) ? { background: 'linear-gradient(135deg,#dc143c,#8f0f2c)', color: '#fff' }
  ...
  if (view === 'result') {
    ...
    return (
      <div className="p-4 pb-24 min-h-full flex flex-col items-center justify-center text-center" style={{ background: 'radial-gradient(120% 80% at 50% 0%, rgba(220,20,60,0.16), transparent 60%)' }}>
        ...
        <div className="grid grid-cols-2 gap-3 w-full max-w-xs mb-8">
          <div className="rounded-2xl p-4" style={{ background: 'rgba(220,20,60,0.14)', border: '1px solid rgba(220,20,60,0.4)' }}>
            <p className="text-2xl font-black text-red-300">+{gainedRef.current.xp}</p><p className="text-[10px] uppercase tracking-widest text-gray-400">XP</p>
          </div>
        ...
        <button onClick={() => setView('map')} className="w-full max-w-xs py-3.5 rounded-2xl text-sm font-black text-white" style={{ background: 'linear-gradient(135deg,#dc143c,#8f0f2c)' }}>
          Continua
        </button>
      </div>
    );
    ...
    <div className="p-4 pb-24 min-h-full">
      <div className="mb-4">
        ...
        <div className="flex items-end justify-between mt-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-red-300/80">Kurs polskiego</p>
            <h1 className="text-2xl font-black text-white leading-none" style={{ fontFamily: 'Russo One, sans-serif' }}>Polski 🇵🇱</h1>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-2xl p-3" style={{ background: 'rgba(220,20,60,0.12)', border: '1px solid rgba(220,20,60,0.3)' }}>
          ...
          <div className="h-2 rounded-full overflow-hidden mb-1" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <div className="h-full rounded-full" style={{ width: `${Math.min(100, (daily.xp / DAILY_GOAL_XP) * 100)}%`, background: 'linear-gradient(90deg,#dc143c,#8f0f2c)' }} />
          </div>
      ...
      <div className="mb-4 rounded-2xl p-3" style={{ background: 'rgba(34,176,125,0.08)', border: '1px solid rgba(34,176,125,0.25)' }}>
        <div className="flex items-center justify-between mb-1">
          <p className="text-[10px] uppercase tracking-widest text-emerald-300">Polish Brain</p>
