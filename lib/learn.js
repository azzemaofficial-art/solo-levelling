...
export function nextQuestion(state, gateOverride) {
  ...
  // ripassa SOLO se la domanda è effettivamente caricata (il pool generato potrebbe non esserlo → evita crash)
  const due = s.srs.find((it) => it.due <= today() && GATES[it.gate] && GATES[it.gate].course[it.qid]);
