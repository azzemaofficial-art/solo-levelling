...
// ── CONTATORE CALCI DA MATCH ─────────────────────────────────────────────────
// Rilevatore incrementale: in pasto due campioni consecutivi {t,kL,kR,...};
// un calcio = estensione esplosiva del ginocchio oltre kickVel °/s.
// `state` ({ lastKickT: { kL, kR } }) viene mutato dal chiamante (refrattario).
export function detectKicks(prev, cur, state, opts = {}) {
  const KICK_VEL = opts.kickVel ?? 500;   // °/s: estensione ginocchio = calcio
  const REFRACT = opts.refractMs ?? 700;  // anti doppi conteggi dello stesso calcio
  const hits = [];
  if (!prev || !cur) return hits;
  const dt = cur.t - prev.t;
  if (dt <= 0 || dt > 250) return hits; // buco di tracking
  if (!state.lastKickT) state.lastKickT = { kL: -1e9, kR: -1e9 };
  for (const j of ['kL', 'kR']) {
    if (prev[j] == null || cur[j] == null) continue;
    const v = ((cur[j] - prev[j]) / dt) * 1000;
    if (v > KICK_VEL && cur.t - (state.lastKickT[j] || -1e9) > REFRACT) {
      state.lastKickT[j] = cur.t;
      hits.push({ side: j, vel: Math.round(v), t: cur.t });
    }
  }
  return hits;
}

// Livello calcio (0-100) dall'esito del match: volume di calci + velocità media
// dei picchi. 0 calci = 0. Tanti calci veloci = livelli alti.
export function kickLevelFromMatch({ tot = 0, avgPeak = 0 } = {}) {
  if (!tot) return 0;
  const volumeScore = Math.min(60, tot * 2.4);                    // 25 calci → 60
  const speedScore = Math.min(40, Math.max(0, (avgPeak - 400) / 15)); // 400°/s→0, 1000°/s→40
  return Math.min(100, Math.round(volumeScore + speedScore));
}
