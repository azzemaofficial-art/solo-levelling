...
// Contesto COMPATTO per il cervello live (visual.js): respirazione + disciplina
// in 2 righe, così il maestro può correggere anche il fiato e l'atteggiamento
// senza appesantire il prompt latency-sensitive. Ritorna '' se disciplina ignota.
export function maestroContext(disciplineId) {
  const d = getDiscipline(disciplineId);
  if (!d) return '';
  return (
    `KNOWLEDGE ${d.name.toUpperCase()}: ` +
    `Respiro → ${d.breathing.principle} ` +
    `Disciplina → ${d.philosophy}`
  );
}

// Un drill della disciplina (per "dammi un esercizio"): sceglie in base a un
// seed deterministico (combo del giorno / rotazione) o casuale.
export function pickDrill(disciplineId, seed) {
  const d = getDiscipline(disciplineId);
  if (!d || !d.techniques.length) return null;
  const idx = seed != null ? Math.abs(seed) % d.techniques.length : Math.floor(Math.random() * d.techniques.length);
  const t = d.techniques[idx];
  return { technique: t.it, drill: t.drill, cues: t.how, mistakes: t.mistakes };
}
