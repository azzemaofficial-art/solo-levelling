...
const MODELS = {
  ...
  'gemini-flash':  { name:'Gemini Flash 1.5',     emoji:'💎', color:'#34d399', provider:'openrouter', envKey:'OPENROUTER_API_KEY',             model:'google/gemini-flash-1.5' },
  ...
  'deepseek-v3':   { name:'DeepSeek V3',          emoji:'🐋', color:'#22d3ee', provider:'openrouter', envKey:'OPENROUTER_API_KEY',             model:'deepseek/deepseek-chat' },
  'deepseek-r1':   { name:'DeepSeek R1',          emoji:'🧠', color:'#818cf8', provider:'openrouter', envKey:'OPENROUTER_API_KEY',             model:'deepseek/deepseek-r1' },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      id: modelId, name: cfg.name, emoji: cfg.emoji, color: cfg.color,
      answer: '?', latencyMs: Date.now() - t0, isCorrect: false, error: 'timeout',
    });
  }
}
